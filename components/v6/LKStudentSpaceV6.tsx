...
function TextsScreen({ actor, show, back }: { actor: V6User; show: (message: string) => void; back: () => void }) {
  ...
  const [prompt, setPrompt] = useState(db.aiPrompts.super_admin ?? "");
  return <div className="space-y-4"><BackHeader title="טקסטים ו־AI" back={back} /><Surface tone="repertoire" className="space-y-3"><FormField label="כותרת כניסה" value={title} onChange={setTitle} /><V6Button onClick={() => { dispatch({ type: "update_text", actor, key: "loginTitle", value: title }); show("הטקסט נשמר"); }}>שמירה</V6Button></Surface><Surface tone="admin" className="space-y-3"><label className="block text-right"><span className="text-[11px] font-bold text-white/46">תבנית AI למנהל מערכת</span><textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} className="mt-2 min-h-32 w-full rounded-[18px] border border-white/[0.065] bg-white/[0.075] p-3 text-right text-[16px] text-white outline-none" /></label><p className="text-right text-xs text-white/45">תבניות AI נכללות בייצוא המסד. פרסום תוכן דורש אישור אנושי.</p><V6Button onClick={() => show("תבנית AI נשמרה מקומית לייצוא הבא")}>שמירת תבנית</V6Button></Surface></div>;
}
