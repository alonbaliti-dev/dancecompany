...
import { Bell, CalendarDays, ClipboardList, ImagePlus, MessageCircle, Receipt, Shield, Sparkles, Trophy } from "lucide-react";
...
import { ActionPill, BidiNumber, Button, FeedRow, HeroSurface, OpenCluster, RtlText, SafeMeta, SafeTitle, StatusBadge, v6Cx, v6Type, type V6Tone } from "@/components/v6/design-system";
...
export function HomeScreen({ user, openScreen, openTab }: { user: V6User; openScreen: (screen: V6Screen) => void; openTab: (tab: V6Tab) => void }) {
  ...
  ].filter(Boolean).slice(0, 4) as HomeAction[];
  const feed = [
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="relative">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
            <div className="min-w-0 flex-1 text-start">
              ...
              <SafeTitle as="h1" className="mt-2 max-w-[21rem] text-[clamp(1.72rem,7.2vw,2.35rem)] font-semibold leading-[1.08] tracking-[-0.050em] text-white">{copy.title}</SafeTitle>
            </div>
          </div>

          <div className="mt-6 max-w-[24rem] text-start">
      ...
      <OpenCluster tone={tone} className="px-5 py-4">
        <div className="mb-3 flex items-center gap-2 px-1 text-start">
          <Sparkles className="shrink-0 text-[#f4d58d]/55" size={17} strokeWidth={1.8} />
          <div className="min-w-0">
            <p className={v6Type.kicker}>פעולות מהירות</p>
            <SafeTitle as="h2" className="mt-1 text-[17px] font-semibold tracking-[-0.035em] text-white/86">מה צריך לפתוח עכשיו</SafeTitle>
          </div>
        </div>
      ...
      {feed.length ? (
        <OpenCluster tone={unread ? "urgent" : "modern"} className="px-5 py-4">
          <p className={v6Type.kicker}>עדכון קצר</p>
          <div className="mt-3">
            {feed.map((item) => <FeedRow key={item.id} {...item} />)}
          </div>
        </OpenCluster>
      ) : null}
    </div>
